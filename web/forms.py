...
class ClienteForm(forms.Form):
    ...
    sexo = forms.ChoiceField(label='Sexo',choices=SEXO_CHOICES,widget=forms.Select(attrs={'class':'form-select'}))
    fecha_nacimiento = forms.DateField(label='Fecha Nac',input_formats=['%Y-%m-%d'],widget=DateInput)